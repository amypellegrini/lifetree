...
const userId = "Amy";
async function createGoal(userId, goal, initialTaskIds = []) {
...
async function createTask(userId, goalId, task, prevTaskIds = [], nextTaskIds = []) {
...
async function updateGoalInitialTasks(goalId, initialTaskIds) {
...
server.tool("create-task", "Create a new Life Tree task for a given goal linked by its goal ID", {
    ...
    const taskId = await createTask(userId, goalId, task, prevTaskIds, nextTaskIds);
    return {
        content: [
            {
                type: "text",
...
server.tool("update-goal-status", "Update the status of a goal", {
...
server.tool("update-task-status", "Update the status of a task", {
    taskId: z.string().describe("The task ID to update"),
    status: z
        .enum(["Not Started", "In Progress", "Done"])
...
server.tool("update-goal-initial-tasks", "Update the initial tasks for an existing goal", {
    goalId: z.string().describe("The goal ID to update"),
    initialTaskIds: z
        .array(z.string())
...
server.tool("delete-task", "Delete a task by its ID", {
